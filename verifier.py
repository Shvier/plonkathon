...
class VerificationKey:
    ...
    def verify_proof(self, group_order: int, pf, public=[]) -> bool:
        ...
        PI_values = [Scalar(-x) for x in public] + [Scalar(0) for _ in range(group_order - len(public))]
        PI = Polynomial(PI_values, Basis.LAGRANGE)
    ...
    def verify_proof_unoptimized(self, group_order: int, pf, public=[]) -> bool:
        ...
        ZH_ev = zeta ** group_order - 1
        # 6. Compute Lagrange polynomial evaluation L_0(ζ)
        L0_ev = Polynomial([Scalar(1)] + [Scalar(0)] * (group_order - 1), Basis.LAGRANGE).barycentric_eval(zeta)
        # 7. Compute public input polynomial evaluation PI(ζ).
        PI_values = [Scalar(-x) for x in public] + [Scalar(0) for _ in range(group_order - len(public))]
        PI = Polynomial(PI_values, Basis.LAGRANGE)
        PI_ev = PI.barycentric_eval(zeta)
        ...
        assert b.pairing(
            ...
            b.add(self.X_2, ec_mul(b.G2, -zeta)), proof["W_z_1"]
        )
        print("done check 1")
        ...
        assert b.pairing(
            ...
            b.add(self.X_2, ec_mul(b.G2, -zeta * omega)), proof["W_zw_1"]
        )
        print("done check 2")
